...
class dotdeb {
  file { '/etc/apt/sources.list':
    source => '/vagrant/.puppet/files/apt.sources.list',
    owner  => 'root',
    group  => 'root'
  }
  exec { 'apt-get update':
...
exec { 'remove-local-xml':
  command     => '/bin/rm /vagrant/app/etc/local.xml',
...
file { '/etc/apache2/mods-available/fastcgi.conf':
  source  => '/vagrant/.puppet/files/fastcgi.conf',
  require => Package['apache2'],
  notify  => Exec['reload-php5-fpm']
}

file { '/etc/php5/fpm/pool.d/www.conf':
  source  => '/vagrant/.puppet/files/www.conf',
  require => Package['php5-fpm'],
  notify  => Exec['reload-php5-fpm']
}
...
file { '/etc/apache2/sites-enabled/001-stats':
  source  => '/vagrant/.puppet/files/stats.conf',
  require => Package['php5-fpm'],
  notify  => Service['apache2']
}
...
file { '/home/vagrant/bin/reinstall':
  owner    => 'vagrant',
  group    => 'vagrant',
  source   => '/vagrant/.puppet/files/reinstall.sh',
  mode     => '0755',
...
file { '/home/vagrant/.bash_aliases':
  owner    => 'vagrant',
  group    => 'vagrant',
  source   => '/vagrant/.puppet/files/bash_aliases',
  mode     => '0755',
  require  => File['/home/vagrant/bin']
}
...
file { "/etc/profile.d/magento.sh":
    ensure => present,
    source => '/vagrant/.puppet/files/magento.d.sh',
}
